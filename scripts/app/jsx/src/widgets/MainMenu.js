...
var MainMenu = React.createClass({
    ...
    render: function () {
        return (
            <nav id="mainMenu" className="navbar">
                <ul className="nav navbar-nav">
                    {this.state.channelList.map(function (item) {
                        return <li id={item.channelPath} key={item.channelID}>
                            <a href={SiteProperties.clientURL + Page[item.channelPath]}>{item.channelName}</a>
                        </li>
                    })}
                </ul>
            </nav>
